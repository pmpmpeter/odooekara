...
class ApplicantInvitation(models.Model):
    ...
    # To get the dynamic subject based on the hr.applicant stage_id
    # @api.depends('stage_id')
    # def _compute_letter_subject(self):
    #     for record in self:
    #         if record.applicant_id and record.stage_id:
    #             stage_name = record.stage_id
    #             # Search for the corresponding record in the recruitment.invitation.letter.config model
    #             subject_record = self.env['recruitment.invitation.letter.config'].sudo().search([
    #                 ('stage_id.name', '=', stage_name)
    #             ], limit=1)
    #             record.letter_subject = subject_record.name if subject_record else "Default Invitation Subject"
    #         else:
    #             record.letter_subject = "Default Invitation Subject"
