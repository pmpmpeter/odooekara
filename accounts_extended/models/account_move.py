        ...
        for move in self.filtered(lambda move: move.partner_id.commercial_partner_id.tcs_applicable):
            ...
            if move.partner_id and move.amount_untaxed_signed>0:
                query1 = """
                        ...
                        """
                query_params1 = (move.partner_id.commercial_partner_id.id, str(fiscal_year_end_date), str(fiscal_year_start_date))
            ...
            if show_warning and tcs_limit and basic_amount > float(tcs_limit_amount):
                ...
                tcs_limit_amount_formatted = formatLang(self.env, float(tcs_limit_amount), currency_obj=move.company_id.currency_id)
                msg = "Cummulative Sales for - %s in %s is %s which is exceeding TCS limit of %s."%(move.partner_id.name, fiscal_year.display_name, basic_amount_formatted, tcs_limit_amount_formatted)
    ...
    def _compute_partner_tds_warning(self):
        ...
        for move in self.filtered(lambda move: move.partner_id.commercial_partner_id.tds_applicable):
            ...
            if show_warning and tds_limit and basic_amount > float(tds_limit_amount):
                ...
                move.partner_id.name, fiscal_year.display_name, basic_amount_formatted, tds_limit_amount_formatted, tds_tax_id.display_name)
