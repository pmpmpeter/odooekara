...
class TeConsolidation(models.Model):
    ...
    def action_update_consolidate_crr(self):
        ...
        budget_lines = self.env['crr.budget.line'].sudo().search([
            ('budget_id', 'in', share_ids.mapped('budget_id').ids), ('budget_id.state', '=', 'to approve')
        ])
    ...
    def action_open_consolidation(self):
        self.ensure_one()
        # line_ids = self.env['crr.budget.line'].sudo().search([('budget_id','in',self.crr_share_line.budget_id.ids),('budget_id.user_type','=','odoo')])
        return {
            'type': 'ir.actions.act_window',
